...
<title>เพิ่มข้อมูลของปริญญานิพนธ์</title>
</head>
...
<center> 
<br /><br />
 <h2>เพิ่มข้อมูลปริญญานิพนธ์</h2>
<br /><br />
<?php 
$sql = "SELECT * FROM book WHERE bname = '' AND rfid != ''"; 
$result=mysqli_query($link,$sql);
$row = mysqli_fetch_array($result);
if(mysqli_num_rows($result) == 0)
{
	echo "<script>alert('ไม่สามารถเพิ่มข้อมูลได้ กรุณาลงทะเบียน RFID Tag เพื่อเพิ่มข้อมูลของปริญญานิพนธ์')</script>";
	echo "<meta http-equiv='refresh' content='0;url=index.php'>";
}
?>
 <div style="max-width:1000px; width:100%;" class="table-responsive"> 
<form action="saveinsert.php?id=<?php echo $row["rfid"]; ?>" method="post">
<table style="border-style:hidden;" class="table-bordered">
<tr> <th style="text-align:left; height:50px; width:35%;"> ลำดับที่ (No.): </th><td> <input class="form-control-static" type="text" value="<?php echo $row["no"]; ?>" name="no" id="no" style="width:100px; float:left; border-radius:5px;" disabled="disabled"/></td></tr>
...
<tr> <th style="text-align:left; height:100px; width:35%;"> ชื่อปริญญานิพนธ์: </th><td> <textarea class="form-control-static" type="text" value="<?php echo $row["bname"]; ?>" title="กรุณากรอกชื่อปริญญานิพนธ์" data-toggle="tooltip" name="bname" id="bname" style="width:100%;  height:100px; border-radius:5px;"/ required="required"><?php echo $row["bname"]; ?></textarea></td></tr>
<tr> <th style="text-align:left; height:100px; width:35%; "> ชื่อผู้จัดทำ: </th><td> <textarea class="form-control-static" type="text" value="<?php echo $row["wname"]; ?>" pattern="[^0-9_]{1,}" title="กรุณากรอกชื่อผู้จัดทำเป็นภาษาไทยและภาษาอังกฤษเท่านั้น" data-toggle="tooltip" name="wname" id="wname" style="width:100%; height:100px; border-radius:5px;" required="required"/><?php echo $row["wname"]; ?></textarea> </td></tr></table>
<br />
